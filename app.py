...
def manage_products():
    if not session.get('is_admin'):
        return redirect(url_for('index'))
    return "Управление товарами (заглушка)"

@app.route('/admin/manage-banners', methods=['GET','POST'])
def manage_banners():
    if not session.get('is_admin'):
        return redirect(url_for('index'))
    if request.method == 'POST':
        ...
        if file:
            ...
            save_path = os.path.join('static','banners', file.filename)
            file.save(save_path)
    return render_template('admin/manage_banners.html')
